#!/bin/bash
########################################################################################################################################
### Call: StartSim_htk_lattice_fixed_noref_NodeTimes.bash FileListPath KnownN UnkN NumIter AmScale PruneFactor                        ##
### e.g.: ./StartSim_htk_lattice_fixed_noref_NodeTimes.bash WSJCAM0_WSJ0+1_Cross_Lattice/WSJCAM0_Phoneme_Lattice.htk.txt 1 2 200 1 16 ##
###                                                                                                                                   ##
...
###                                                                                                                                   ##
### Segmentation results are saved in Results/${Path}_htk_fixed_noref/${FileList}.                                                    ##
###                                                                                                                                   ##
...
Prefix="WLM${2}_CLM${3}_IT${4}_AS${5}_P${6}_MWL${7}"
PostFix='_htk_fixed_noref_NodeTimes'
...
BeamWidth="-BeamWidth 20"
echo ${7}
#WordLengthModulation='-WordLengthModulation ${7}'                                           # Set word length modulation. -1: off, 0: automatic, >0 set mean word length (-WordLengthModulation WordLength (-1))
...
PruneFactor="-PruneFactor ${6}"                                                           # Prune paths in the input that have a PruneFactor times higher score than the lowest scoring path (-PruneFactor X (inf))
...
/mnt/matylda5/iondel/workspace/2017/JSALT/Tools/LatticeWordSegmentation/build/LatticeWordSegmentation ${KnownN} \
                          ${UnkN} \
                          ${NoThreads} \
                          ${PruneFactor} \
                          ${InputFilesList} \
